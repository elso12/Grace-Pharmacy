...
export const checkVelocityAndGeneratePOs = async (): Promise<void> => {
  ...
  try {
      ...
      if (daysOfStock < 15) {
        ...
        if (latestBatch && latestBatch.supplier) {
          ...
          posToGenerate.get(supplierId)!.push({
            ...
            expectedPrice: latestBatch.purchasePrice
          });
        }
      }
    }
    ...
    for (const [supplierId, items] of posToGenerate.entries()) {
      const totalValue = items.reduce((sum, item) => sum + (item.quantityOrdered * (item.expectedPrice || 0)), 0);
      ...
      const newPO = new PurchaseOrder({
        ...
        notes: 'Auto-generated by Velocity Service based on 30-day run rate.'
      });
